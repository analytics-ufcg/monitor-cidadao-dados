...
```{r, include=FALSE}
contratos <- read_csv("Contratos-Sagres2017.csv", 
...
# Recupera tabela de Codigo Funcao
# codigo_funcao = tbl(sagres, 'Codigo_Funcao') %>%
#   collect(n = Inf)
# 
# write.csv(codigo_funcao, file = "CodigoFuncao-Sagres2017.csv", row.names = FALSE)
```
...
# Recupera tabela de Codigo Funcao
# codigo_subfuncao = tbl(sagres, 'Codigo_Subfuncao') %>%
#   collect(n = Inf)
# 
# write.csv(codigo_subfuncao, file = "CodigoSubFuncao-Sagres2017.csv", row.names = FALSE)
```

```{r, include=FALSE}
# Codigo_ElementoDespesa
# Recupera tabela de Codigo Funcao
# codigo_elemento = tbl(sagres, 'Codigo_ElementoDespesa') %>%
#   collect(n = Inf)
# 
# write.csv(codigo_elemento, file = "CodigoElemento-Sagres2017.csv", row.names = FALSE)

# Codigo_Subelemento
# Recupera tabela de Codigo Funcao
# codigo_subelemento = tbl(sagres, 'Codigo_Subelemento') %>%
  # collect(n = Inf)
# 
# write.csv(codigo_subelemento, file = "CodigoSubElemento-Sagres2017.csv", row.names = FALSE)
```
...
# Dados de Licitações
licitacoes <- read_csv("Licitacoes-Sagres2017.csv", 
    col_types = cols(cd_UGestora = col_character(), 
...
empenhos_ass_contratos <- read_csv("EmpenhosAssoContratos-Sagres2017.csv", 
    col_types = cols(cd_Acao = col_character(), 
        cd_CatEconomica = col_character(), 
        cd_Funcao = col_character(), 
        cd_Modalidade = col_character(), 
...
O valor total do contrato e sua data de encerramento são dados disponíveis no SAGRES. Para que estejamos todos na mesma página, vamos definir um *contrato vigente* como um contrato ainda em seu prazo de validade. Dada a versão do SAGRES utilizada aqui, nesta análise iremos considerar como um contrato vigente como **o contrato cuja data de expiração é supeior a 01 de janeiro de 2017**.

<br>

```{r}
contratos_vigentes_f %>% 
ggplot(aes(x = pr_Vigencia, y = vl_TotalContrato)) + 
  geom_line(color = "coral2") + 
  geom_point(color = "firebrick3", alpha = 0.3) +
  labs(x = "Ano do encerramento do contrato", y = "Valor da licitação", subtitle = "Valor e ano de finalização de contratos")
```

<br>

Acima, estão expostos os valores totais dos contratos em relação a sua data de término. Vemos que a grande maioria dos contratos possui validade até 2020, mas também existem contratos com datas de término que podem chegar ao ano de 2030. Compreender a prática administrativa de gerar contratos tão "largos" pode servir de insumo para nortear outras análises ou percepções.
...
```{r}
contratos_vigentes_f %>% 
ggplot(aes(x = tempo_Vida, y = vl_TotalContrato)) + 
  geom_line(color = "coral2") + 
  geom_point(color = "firebrick3", alpha = 0.3) +
  labs(x = "Dias", y = "Valor Total", subtitle = "Período de Vigência dos Contratos")
```
...
empenhos_ass_contratos_vigentes <- read_csv("EmpenhosAssoContratosVigentes-Sagres2017.csv", 
...
funcao <- read_csv("CodigoFuncao-Sagres2017.csv", 
    col_types = cols(cd_Funcao = col_character()))
  
subfuncao <- read_csv("CodigoSubFuncao-Sagres2017.csv", 
    col_types = cols(cd_SubFuncao = col_character()))
  
elemento <- read_csv("CodigoElemento-Sagres2017.csv", 
    col_types = cols(cd_Elemento = col_character()))

subelemento <- read_csv("CodigoSubElemento-Sagres2017.csv", 
...
empenhos_ass_contratos_vigentes %>% 
  ...
  summarise(volume_Subelemento = n()) %>% 
...
contratos_empenhos_longos %>% 
  group_by(de_Funcao) %>% 
  summarise(montanteEmpenho = sum(vl_Empenho)) %>%
  na.omit() %>% 
...
--------
          ...
          Dados Extraídos do Sagres 2017.
