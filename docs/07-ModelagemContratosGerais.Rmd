...
# versão mais simples
treino_part <- treino_exc %>% 
  slice(1:20000)


rf_fit <- random_forest_m %>% 
  fit(status_tramita ~ ., data = treino_part)
...
rf_smote_class_pred <- rf_fit_smote %>% predict(teste %>% select(-exc_features)) %>% 
  mutate(rf_smote = .pred_class) %>% select(-.pred_class)

teste <- teste %>% 
  cbind(rf_smote_class_pred)

# Calcula as métricas avaliativas para a previsão
av_rf_smote <- avaliacao("Floresta Al. Smote", teste, "rf_smote", "status_tramita")
```
```{r}
av_rf_smote
```
