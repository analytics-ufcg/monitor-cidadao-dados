...
test_that("Is dataframe", {
  expect_true(is.data.frame(participantes))
})

test_that("Not Empty", {
  expect_true(nrow(participantes) != 0)
})

test_that("fetch_participantes()", {
  expect_true(all(sapply(participantes, class) %in% COLNAMES_PARTICIPANTES))
})
