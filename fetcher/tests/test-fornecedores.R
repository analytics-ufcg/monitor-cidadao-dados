...
test_that("Is dataframe", {
  expect_true(is.data.frame(fornecedores))
})

test_that("Not Empty", {
  expect_true(nrow(fornecedores) != 0)
})

test_that("fetch_fornecedores()", {
  expect_true(all(sapply(fornecedores, class) %in% COLNAMES_FORNECEDORES))
})
